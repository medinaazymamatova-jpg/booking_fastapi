...
from myproject.database.schema import UserProfileInputSchema, UserProfileLoginSchema
from passlib.context import CryptContext
from datetime import timedelta, datetime, timezone
from typing import Optional
from jose import jwt
from myproject.config import ALGORITHM, ACCESS_TOKEN_LIFETIME, REFRESH_TOKEN_LIFETIME,SECRET_KEY
...
@auth_router.post('/register', response_model=UserProfileInputSchema)
...
async def logout(refresh_token: str, db: Session = Depends(get_db)):
    ...
    db.commit()
    return {'massage': 'Сайттан чыктыныз'}

@auth_router.post('/refresh')
async def refresh(refresh_token: str, db: Session = Depends(get_db)):
    stoped_token = db.query(RefreshToken).filter(RefreshToken.token == refresh_token).first()
    if not stoped_token:
        raise HTTPException(detail='Refresh token туура эмес', status_code=401)

    access_token = create_access_token({'sub': stoped_token.id})
